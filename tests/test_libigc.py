...
class TestNapretTaskParsing(unittest.TestCase):
    ...
    def testTaskHasTurnpointsWithRadius(self):
        self.assertGreaterEqual(
            min(map(lambda turnpoint: turnpoint.radius, self.task.turnpoints)), 0.2
        )
        self.assertLessEqual(
            max(map(lambda turnpoint: turnpoint.radius, self.task.turnpoints)), 4
        )

    def testTaskHasTurnpointsWithLatitude(self):
        self.assertEqual(
            max(map(lambda turnpoint: int(turnpoint.lat / 46), self.task.turnpoints)), 1
        )

    def testTaskHasTurnpointsWithLongitude(self):
        self.assertEqual(
            max(map(lambda turnpoint: int(turnpoint.lon / 12), self.task.turnpoints)), 1
        )
...
class TestOlsztynFlightParsing(unittest.TestCase):
    ...
    def testMetadataIsCorrectlyRead(self):
        ...
        self.assertEqual(
            self.flight.i_record, "I073638FXA3941ENL4246TAS4751GSP5254TRT5559VAT6063OAT"
        )
        # 2011-09-02 0:00 UTC
        self.assertAlmostEqual(self.flight.date_timestamp, 1314921600.0)
        self.assertEqual(self.flight.glider_type, "test_glider_xx")
        self.assertEqual(self.flight.competition_class, "some_competition_class")
        self.assertEqual(self.flight.fr_firmware_version, "2.2")
        self.assertEqual(self.flight.fr_hardware_version, "2")
        self.assertEqual(self.flight.fr_recorder_type, "LXNAVIGATION,LX8000F")
        self.assertEqual(self.flight.fr_gps_receiver, "uBLOX LEA-4S-2,16,max9000m")
        self.assertEqual(self.flight.fr_pressure_sensor, "INTERSEMA,MS5534A,max10000m")
...
class TestWhichFlightToPick(unittest.TestCase):
    ...
    def testConcatIsLongerThanFirst(self):
        ...
        # Takeoff is the same
        self.assertEqual(
            flight_first.takeoff_fix.timestamp, flight_concat.takeoff_fix.timestamp
        )
